...
export default function Partners() {
  ...
  return (
    <section id="partners" className="py-25 bg-white">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid grid-cols-6 gap-5 mb-16 max-md:grid-cols-3 max-sm:grid-cols-2">
          {partners.logos.map((logo, i) => (
            <div className="flex flex-col items-center gap-2.5 p-6 bg-off-white rounded-card border border-gray-200 transition-all duration-300 hover:border-primary hover:shadow-card-hover hover:-translate-y-0.5" key={i}>
              ...
              <span className="text-[0.78rem] text-gray-700 font-medium text-center">{logo.name}</span>
            </div>
          ))}
        </div>
          ...
          <div>
            ...
            <ul className="list-none mb-7">
              {partners.benefits.map((b, i) => (
                <li key={i} className="flex items-center gap-2.5 py-2 text-gray-700 text-[0.95rem]">
