...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="py-25 bg-white">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="flex justify-center gap-2 mb-10 flex-wrap">
          {gallery.categories.map((cat, i) => (
            <button
              key={i}
              className={`py-2 px-5 border-2 border-gray-200 bg-transparent rounded-full text-[0.88rem] font-medium cursor-pointer transition-all duration-300 text-gray-700 ${activeCategory === cat ? "bg-primary text-white border-primary" : "hover:border-primary hover:text-primary"}`}
        ...
        <div className="grid grid-cols-4 gap-5 max-md:grid-cols-2 max-sm:grid-cols-1">
          {filtered.map((item, i) => (
            <div className="rounded-card overflow-hidden border border-gray-200 transition-all duration-300 hover:shadow-card-hover hover:-translate-y-1" key={i}>
              <div
              ...
              <div className="p-4">
                ...
                <span className="text-[0.75rem] bg-gray-100 text-gray-700 py-[3px] px-2.5 rounded-2xl font-medium">{item.category}</span>
              </div>
            </div>
          ))}
        </div>
