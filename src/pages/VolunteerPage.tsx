        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-6 max-md:grid-cols-1">
            {volunteer.whyVolunteer.items.map((item, i) => (
              <div className="bg-white rounded-card p-8 shadow-card-hover text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-card-lg" key={i}>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <ul className="list-none grid grid-cols-2 gap-3 max-w-[600px] mx-auto mb-6 max-md:grid-cols-1">
            {volunteer.corporate.benefits.map((benefit, i) => (
              <li key={i} className="bg-gray-100 py-3 px-4 rounded-card-sm text-[0.95rem] text-secondary text-center">{benefit}</li>
            ))}
          </ul>
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-6 max-md:grid-cols-1">
            {volunteer.profiles.map((profile, i) => (
              <div className="bg-white rounded-card p-8 shadow-card-hover text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-card-lg" key={i}>
                ...
                <p className="italic text-gray-700 text-[0.9rem] leading-[1.5] mt-3">&ldquo;{profile.quote}&rdquo;</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(400px,1fr))] gap-6 max-md:grid-cols-1">
            {volunteer.alumni.map((alumnus, i) => (
              <div className="bg-white rounded-card p-8 shadow-card-hover border-l-4 border-accent" key={i}>
                ...
                <p className="italic text-gray-700 text-[0.9rem] leading-[1.5]">&ldquo;{alumnus.quote}&rdquo;</p>
              </div>
            ))}
          </div>
        </div>
